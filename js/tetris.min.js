...
b.textContent="Hide"):(a.classList.add("hidden"),b.textContent="Show")},I=()=>{Array.from(document.querySelectorAll(".flashOn")).forEach(a=>{a.classList.remove("flashOn")})},Y=a=>{for(const b of a)if(19===b[1]||2===e[b[0]][b[1]+1])return!1;return!0},D=()=>{if(r){const a=[],b=[];p().forEach(d=>{const [f,h]=m(d.id);a.push([f,h]);b.push([f,h])});let c=0;for(const d of a)d[1]>c&&(c=d[1]);for(;Y(a);)a.forEach(d=>{d[1]+=1});wa(b,a)?I():(I(),a.forEach(d=>{document.getElementById(d[0]+"-"+d[1]).firstChild.classList.add("flashOn")}))}},
wa=(a,b)=>{for(const c of a)for(const d of b)if(c[0]===d[0]&&c[1]===d[1])return!0;return!1};(()=>{for(var a=0;10>a;a++){var b=document.createElement("div");b.id="col-"+a;b.classList.add("col");ba.appendChild(b);e.push([]);for(var c=0;20>c;c++){var d=document.createElement("div");d.id=a+"-"+c;d.classList.add("cell");b.appendChild(d);e[a].push(0);var f=document.createElement("div");f.id="f"+a+"-"+c;f.classList.add("flash");d.appendChild(f)}}b=document.getElementById("nextPeice");a=document.getElementById("heldPeice");
for(c=0;4>c;c++)for(d=document.createElement("div"),d.id="col-"+c+"-n",d.classList.add("col"),b.appendChild(d),f=0;4>f;f++){const h=document.createElement("div");h.id=c+"-"+f+"-n";h.classList.add("cell");d.appendChild(h)}for(b=0;4>b;b++)for(c=document.createElement("div"),c.id="col-"+b+"-h",c.classList.add("col"),a.appendChild(c),d=0;4>d;d++)f=document.createElement("div"),f.id=b+"-"+d+"-h",f.classList.add("cell"),c.appendChild(f);document.getElementById("startBtn").addEventListener("click",na);document.addEventListener("keydown",
ea);document.addEventListener("keyup",ma);document.getElementById("hideBtn").addEventListener("click",va);document.getElementById("points").textContent=l;document.getElementById("highPoints").textContent=0<G?G:" No High Score";window.onkeydown=h=>!((" "===h.key||"ArrowUp"===h.key||"ArrowDown"===h.key)&&h.target===document.body)})()})();
